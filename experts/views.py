    ...
    if request.method == 'POST':
        ...
        if user is not None:
            if user.is_active:
             ...
             message ='<div class="alert alert-danger" > <button type="button" class="close" data-dismiss="alert" aria-label="Close">&times;</button> nom d\'utilisateur ou mot de passe incorrecte </div>'
...
def lesexperts(request):	
	...
	counthuman = human.count()
	#ceci c'est quand l'on fait une recherche
	if request.method== 'POST':
	    human = User.objects.filter(username__contains=request.POST['username'])
	return render_to_response("../templates/listexpert.html",{'currentuser':currentuser,'counthuman':counthuman,'human': human}, context_instance=RequestContext(request))
...
def listentreprise(request):	
	...
	countentreprises = lesentreprises.count()
	return render_to_response("../templates/listentreprise.html",{'currentuser':currentuser,
	'countentreprises':countentreprises,
	'lesentreprises': lesentreprises}, context_instance=RequestContext(request))
...
def lesoffres(request):	
	...
	return render_to_response("../templates/listeoffre.html",{'currentuser':currentuser, 'countoffres':countoffres,'lesoffres':lesoffres}, context_instance=RequestContext(request))
